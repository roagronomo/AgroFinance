...
export default function AlertasVencimentos() {
  ...
  const carregarAlertas = async () => {
    try {
      setIsLoading(true);
      const servicos = await base44.entities.OutroServico.list("-data_vencimento_boleto");
      ...
      // Filtrar apenas serviços com boleto emitido, não concluídos e com vencimento próximo
      const servicosComAlerta = servicos.filter(s => {
      ...
      servicosComAlerta.sort((a, b) => {
        const dataA = new Date(a.data_vencimento_boleto);
        const dataB = new Date(b.data_vencimento_boleto);
        return dataA - dataB;
      });
      
      setAlertas(servicosComAlerta);
    } catch (error) {
    ...
    <Card className="border-0 shadow-sm">
      ...
      <CardContent className="p-0">
        <div className="max-h-96 overflow-y-auto">
          {alertas.map((servico, idx) => {
            const diasRestantes = calcularDiasRestantes(servico.data_vencimento_boleto);
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex flex-wrap items-center gap-2 text-xs">
                      <span className="flex items-center gap-1 text-gray-500">
                        ...
                        {format(new Date(servico.data_vencimento_boleto), "dd/MM/yyyy", { locale: ptBR })}
                      </span>
